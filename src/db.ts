...
import { ISku, IUser, ICart, UserRole, SkuStatus, Pick, IStar } from './types'
...
try {
  ...
  if (json.star) store.star = json.star
} catch (error) {}
...
export function catsearch(cat: string) {
  let sku = store.skus.filter(item => {
    if(item.cat){
      if(item.cat.indexOf(cat) > -1)
      return item
    }
  }) 
  return sku
...
// 添加收藏
export async function addStar(username: string, skuId: string) {
  let result = store.star.find(item => item.username === username && item.skuId === skuId)
  if (!result) {
    let item = {
      username,
      skuId
    }
    store.star.push(item)
    await dump()
    return item
  }
  return result
...
export function listStar(username: string) {
  let ids = store.star.filter(item => item.username === username).map(item => item.skuId)
  let rows = store.skus.filter(item => ids.includes(item.id))
  return rows
}

// 取消收藏
export async function removeStar(username: string, ids: string[]) {
  store.star = store.star.filter(item => !(item.username === username && ids.includes(item.skuId) === true))
  await dump()
}
