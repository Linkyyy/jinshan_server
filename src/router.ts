...
router.use(async (ctx, next) => {
  ...
  let needAuth = ['/api/cart', '/api/admin', '/api/star']
...
router.post('/cat/search', ctx => {
  ...
  let data = service.catsearch(value.cat)
  if (data) {
    ctx.body = {
      stat: 'OK',
      data
...
router.post('/star/add', async ctx => {
  ...
  let result = await service.addStar(ctx.state.userInfo.username, value.skuId)
  ctx.body = {
    stat: 'OK',
    data: result
  }
...
router.post('/star/list', ctx => {
  let rows = service.listStar(ctx.state.userInfo.username)
  ctx.body = {
    stat: 'OK',
    rows
  }
...
router.post('/star/remove', async ctx => {
  ...
  await service.removeStar(ctx.state.userInfo.username, value.ids)
  ctx.body = {
    stat: 'OK'
  }
})
...
router.post('/admin/sku/add', async ctx => {
  let schema = Joi.object({
    ...
    cat: Joi.array().items(Joi.string().required()).min(1).max(10).required(),
    label: Joi.array().items(Joi.string().required()).min(1).max(10).required()
  })
  let { value, error } = schema.validate(ctx.request.body)
  if (error) {
    return ctx.body = {
...
router.post('/admin/sku/update', async ctx => {
  let schema = Joi.object({
    ...
    cat: Joi.array().items(Joi.string().required()).min(1).max(10),
    label: Joi.array().items(Joi.string().required()).min(1).max(10)
  })
  let { value, error } = schema.validate(ctx.request.body)
  if (error) {
    return ctx.body = {
